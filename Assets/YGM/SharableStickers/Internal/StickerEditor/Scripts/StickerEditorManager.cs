...
namespace YGM.SharableStickers
{
    ...
    public class StickerEditorManager : UdonSharpBehaviourWithUtils
    {
        ...
        public StickerEditor FindOrCreateEditor(
            string stickerId,
            string content,
            Color color,
            Vector3 position,
            ...
            )
        {
            ...
            if (tf != null)
            {
                ...
                if (editor != null)
                {
                    editor.Setup(stickerId, content, color, position, rotation, m_system, initialViewMode, eventListener, onCloseEventName, this, nameof(OnCloseEditor));
                    return editor;
                }
                else
                {
